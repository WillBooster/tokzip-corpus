...
const ALLOWED_LICENSES = new Set(['Apache-2.0', 'BSD-3-Clause', 'MIT', 'MIT-like (curl)', 'MIT/Apache-2.0']);
...
function main(): void {
  ...
  const samplesByLanguage = new Map<string, { content: string; label: string; split?: string }[]>();
  let samples = 0;
  for (const language of readdirSync(CORPUS_DIR, { withFileTypes: true }).filter((entry) => entry.isDirectory())) {
    ...
    for (const origin of ['human', 'llm']) {
      ...
      for (const file of readdirSync(originDir)) {
        if (!referenced.has(`${origin}/${file}`)) errors.push(`${language.name}/${origin}/${file}: not in manifest`);
      }
    }
  }
  for (const entries of entriesByHash.values()) {
    if (new Set(entries.map((entry) => entry.split)).size > 1) {
      errors.push(`exact content crosses train/bench: ${entries.map((entry) => entry.label).join(', ')}`);
    }
  }
  for (const entries of entriesByProvenance.values()) {
    if (new Set(entries.map((entry) => entry.split)).size > 1) {
      errors.push(`upstream file crosses train/bench: ${entries.map((entry) => entry.label).join(', ')}`);
    }
  }
  ...
  console.log(`corpus compliance: ${samples} samples have approved licenses, notices, pins, and hashes`);
}

function validateEntry(dir: string, label: string, entry: ManifestEntry, errors: string[]): void {
  const samplePath = join(dir, entry.file);
  if (!ALLOWED_LICENSES.has(entry.license)) errors.push(`${label}: disallowed license ${entry.license}`);
  if (!parsePinnedSource(entry.source)) errors.push(`${label}: source is not pinned to a 40-character commit`);
  ...
  if (entry.trainable !== true) errors.push(`${label}: sample is not explicitly approved for training`);
...
function validateSourcePolicy(label: string, entry: ManifestEntry, samplePath: string, errors: string[]): void {
  ...
  if (entry.license !== policy.license) errors.push(`${label}: license does not match source policy ${policy.license}`);
  if (entry.trainable !== policy.trainable) errors.push(`${label}: trainable flag does not match source policy`);
  ...
  if (policy.excludePrefixes?.some((excluded) => sourcePath === excluded || sourcePath.startsWith(`${excluded}/`))) {
